...
for message in st.session_state.messages:
    with st.chat_message(message["role"], avatar=Elysia_img if message["role"] == "assistant" else None):
        st.markdown(message["content"])
    

st.markdown("---")

uploaded_img = st.file_uploader("", type=["jpg", "jpeg", "png"], key="elysia_img_upload")
...
if prompt := st.chat_input("메시지를 입력하세요"):
    ...
    # OpenAI 답변 생성
    with st.chat_message("assistant", avatar=Elysia_img):
